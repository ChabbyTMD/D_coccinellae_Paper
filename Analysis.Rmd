...
```{r}
WaspData=read_excel("morph_data_bk.xlsx")
...
WaspPCA <- prcomp(WaspData[,c(4:9,14:19)], center=T, scale. = T)
...
WaspoffspringPCA <- prcomp(WaspData[,c(14:19)], center=T, scale. = T)
...
waspoffspring.nmds = metaMDS(WaspData[,c(14:19)], distance = "bray", k = 2, trace = 0)
# Construct ordination plot.

wasp_ord = ordiplot(waspoffspring.nmds, type = "none",
                    main = paste0("Stress = ", round(waspoffspring.nmds$stress, dig=3)),
                    las = 1)


# Define vectors for Habitat levels and colors. These will be used to layer the points for the 6 habitats in the NMDS plot. The colors are generated using the brewer.pal function that is part of the RColorBrewer package. It generates a palate of colors upon taking in 2 parameters, number of colors to generate and pallet name. Pallet name can be whatever...

waspcolors<-brewer.pal(nlevels(as.factor(WaspData$o.host)),"Set3")
wasplevels<-levels(as.factor(WaspData$o.host))

# This for loop draws points for each of the 6 habitat levels with distinct colors.

for (species in 1:nlevels(as.factor(WaspData$o.host))){
  points(wasp_ord, WaspData$o.host == wasplevels[species],what = "sites", pch = 20, 
      col = waspcolors[species])
}

# Drawing ellipsoid hulls around the habitats.

ordiellipse(waspoffspring.nmds, kind = "ehull", 
            draw = "polygon",
            groups = WaspData$o.host,
            col = brewer.pal(nlevels(as.factor(WaspData$o.host)),"Set3"),
            label = F)
# Drawing habitat legend.
legend("bottomleft",
       legend = levels(as.factor(WaspData$o.host)),
       col = brewer.pal(nlevels(as.factor(WaspData$o.host)),"Set3"),
       pch = 20,
       cex = 0.5,
       title = "Species")
