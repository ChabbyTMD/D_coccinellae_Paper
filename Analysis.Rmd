...
# wasp_rda_plot
# R2 for the model fit for the constrained ordinations. This represents the relative and unadjusted contribution of each eigenvector and is therefore biased
R2 <- RsquareAdj(wasp.rda)$r.squared
R2 

# adjusted R^2. This measures the unbiased amount of explained variation and therefor our full RDA model explains 13.1% of the total variance in the data. 
...
anova.cca(wasp.rda, permutations = 999) #p=0.079 we fail to reject the null hypothesis, the full RDA model with all parent morphological characteristics and parent and offspring hosts is not significant.
# Axis significance
...
#                            p.hd                            p.hl 
#                        2.493576                        3.502987 
#    parent_hostC. septempunctata        parent_hostH. convergens 
#                        1.223503                        1.516912 
# offspring_hostC. septempunctata     offspring_hostH. convergens 
#                        1.405598                        1.289031 


# Parent and offspring beetle host species are not colinear with other predictors. However parent wing length is the only morphometric that does not exhibit colinearirty among other predictors.
# This implies that all other parent morphological characteristics exhibit higher than acceptable levels of colinearity (>2) with other predictors.



# In order to create our reduced RDA model we need to perform forward selection using the forward.sel() function. 
# It is important to note that forward selection can only be performed if the global model i.e. the model that contains all # nolint
# explanatory variables is significant. If it is significant go ahead and perform forward selection.
...
# Inspect for colinearity before creating a simpler model. Note that we are looking for VIF lower than 20. or squareroot of VIF < 2
...
wasp.rda2 <- rda(offspring.center~p.td+p.hd+offspring_host, data = parent.center)
summary(wasp.rda2)
...
# adjusted R^2. This measures the unbiased amount of explained variation and therefor our full RDA model explains % of the variation in the data. 
R2adj <- RsquareAdj(wasp.rda)$adj.r.squared
R2adj 
...
parentwasp<-WaspData[,4:9]
offspring_standard <- decostand(offspringwasp, method = "standardize")
```


# RDA Method 2

## Data import and initial data carpentry. 

We import the wasp morphometric data and subset the offspring and parent morphometrics into 2 dataframes. 
We then proceed to center and scale the data prior to constructing a full RDA model that contains the centered and scaled offspring morphometrics as our dependent response matrix and centered and scaled parent morphology as our independent predictor matrix along with parent and offspring bettle host as catergorical predictors.
```{r}
...
Next few chunks adapted from RDA tutorial : https://pmassicotte.github.io/stats-denmark-2019/07_rda.html#/recipe
...
# adjusted R^2. This measures the unbiased amount of explained variation and therefor our full RDA model explains ~13.8% of the total variance in the data. 
R2adj <- RsquareAdj(wasp.rda)$adj.r.squared
R2adj 


R2.2 <- RsquareAdj(wasp.rda2)$r.squared
R2.2 #0.2447329
R2adj.2 <- RsquareAdj(wasp.rda2)$adj.r.squared
R2adj.2 #0.1584167
```

## RDA model assessment.

For an RDA we need to test for the significance of 3 things. Global RDA significance, Axis significance, Explanatory variable significance.
```{r}

# Global RDA significance
#  Performs a permutation test with 999 permutations and assess the F value to determine whether it is significant or not

anova.cca(wasp.rda, permutations = 999) 

#p=0.079 we fail to reject the null hypothesis, the full RDA model with all parent morphological characteristics and parent and offspring hosts is not significant.
...
# Wasp RDA 2 reduced
anova.cca(wasp.rda2, permutations = 999) #model is significant 0.008
anova.cca(wasp.rda2, by = 'axis') #RDA axis 1 is significant
anova.cca(wasp.rda2, by = 'terms') # parent thorax depth and offspring host are signficant
```

## Model significance
For canonical analysis, model significance relies on permutation tests.
The principle of a permutation test is to generate a reference distribution of the chosen statistic under the null hypothesis H0 by randomly permuting appropriate elements of the data a large number of times and recomputing the statistic each time. 
Then, one compares the true value of the statistic to this reference distribution. 
The p value is computed as the proportion of the permuted values equal to or larger than the true (unpermuted) value of the statistic for a one-tailed test in the upper tail, like the F test used in RDA (Borcard, Gillet, and Legendre 2011).

null hypothesis the observed results can be produced by random chance. we reject the null hypothesis when the p value is less than 0.05

## Predictor colinearity

```{r}
# A sqaure root of VIF value of less than 2 is desirable.
sqrt(vif.cca(wasp.rda))
```

Parent wing length, and all offspring and parent host species are 

## Forward selection of full RDA model
```{r}
# forward selection of explanatory variables

# Forward selection
fs <- forward.sel(offspring.center, # Y matrix
  parent.center[1:6], # X matrix
  adjR2thresh = R2adj, # Set the adj.R2 threshold
  alpha = 0.001, # Set alpha level
  nperm = 999  # Number of permutations
)
fs
# Forward selection failed as p value for variable 1 is greater than 0.001 i.e. 0.019
...
```{r}
# unexplained variance by looking at residual variation in the data based on Kaiser-Guttman criterion
wasp.rda2$CA$eig[wasp.rda2$CA$eig > mean(wasp.rda2$CA$eig)]
```
```{r}
```
```{r}
...
# adjusted R^2. This measures the unbiased amount of explained variation and therefor our full RDA model explains 13% of the variation in the data. 
R2adj <- RsquareAdj(wasp.rda)$adj.r.squared
R2adj 
...
# adjusted R^2. This measures the unbiased amount of explained variation and therefor our full RDA model explains 21.19% of the variation in the data. 
R2adj <- RsquareAdj(wasp.rda2)$adj.r.squared
R2adj #21.19%
# Construct ordination plot with reduced RDA model.
ordiplot(wasp.rda2, scaling = 2, type = "none", cex = 10, xlab = "RDA1", ylab = "RDA2", cex.lab = 1.25)
