...
UniHeadLength <- ggplot(UniSorted, aes(x = p.hl, y = o.hl, color = p.host)) +
    geom_point() +
    geom_smooth(method = lm, se = F) +
    xlab("P Head Length") +
    ylab("O Head Length") +
    ggtitle("Unilineal Head Length Heritability")
...
ggplot(data = WaspData, aes(x = p.wl, y = o.wl, color = p.host)) +
    ...
    stat_poly_eq(use_label(c("eq", "R2")), size = 9) +
    labs(
        x = "Parent Wing Length (mm)",
        y = "Offspring Wing Length (mm)"
        # color = "Type"
